...
function Products() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => [product.name, product.id, product.category, product.material, ...(product.seo?.keywords || [])].some((value) => value?.toLowerCase().includes(term)))
  }, [query])
  ...
  return <div className="min-h-screen bg-stone-50 text-stone-900">
    ...
    <main className="mx-auto max-w-7xl px-6 pb-24 pt-36 lg:px-10">
      ...
      <p className="py-8 text-sm text-stone-500">Showing {filteredProducts.length} catalogue items in the current data set.</p>
