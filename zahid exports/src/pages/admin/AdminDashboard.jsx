...
const sections = [
  { title: 'Products', copy: 'Add, edit, publish and archive catalogue products.', path: '/admin/products', status: 'Live' },
...
function AdminDashboard() {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      <section className="admin-section-grid">{sections.map((section) => <article key={section.title}><span>{section.status}</span><h2>{section.title}</h2><p>{section.copy}</p><Link to={section.path}>Open module →</Link></article>)}</section>
