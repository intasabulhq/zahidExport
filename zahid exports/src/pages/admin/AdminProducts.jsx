...
function AdminProducts() {
  ...
  const archiveProduct = async (product) => {
    ...
    try {
      ...
      setProducts((current) => status && status !== 'archived' ? current.filter((item) => item.id !== product.id) : current.map((item) => item.id === product.id ? { ...item, status: 'archived' } : item))
    } catch (requestError) {
      setError(requestError.message)
    } finally {
      setBusyId('')
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-products-main">
      ...
      {loading ? <div className="admin-products-empty">Loading products…</div> : products.length === 0 ? <div className="admin-products-empty"><h2>No products found</h2><p>Create the first database-backed product.</p><Link to="/admin/products/new">Add product →</Link></div> : <div className="admin-products-table-wrap"><table className="admin-products-table"><thead><tr><th>Product</th><th>SKU</th><th>Status</th><th>Updated</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{products.map((product) => {
        const image = imageUrl(product.images?.[0])
        return <tr key={product.id}><td><div className="admin-product-identity">{image ? <img src={image} alt="" /> : <span className="admin-product-image-placeholder">ZE</span>}<div><strong>{product.name}</strong><small>/{product.slug}</small></div></div></td><td>{product.sku}</td><td><select aria-label={`Status for ${product.name}`} value={product.status} disabled={busyId === product.id} onChange={(event) => updateStatus(product, event.target.value)}><option value="draft">{statusLabels.draft}</option><option value="published">{statusLabels.published}</option><option value="archived">{statusLabels.archived}</option></select></td><td>{new Date(product.updated_at).toLocaleDateString()}</td><td><div className="admin-row-actions"><Link to={`/admin/products/${product.id}/edit`}>Edit</Link><button type="button" disabled={busyId === product.id || product.status === 'archived'} onClick={() => archiveProduct(product)}>Archive</button></div></td></tr>
      })}</tbody></table></div>}
    </main>
  </div>
}
