  ...
  for (const [position, image] of images.entries()) {
    ...
    await client.query(
      `INSERT INTO product_images
        (product_id, url, public_id, alt_text, position)
       VALUES ($1, $2, $3, $4, $5)`,
      [
        productId,
        imageData.url,
...
function sendWriteError(error, res) {
  if (error.code === "23505") {
    ...
    res
      .status(409)
      .json({ error: `A product with this ${field} already exists` });
    return true;
  }
  if (error.code === "23503") {
...
router.get("/admin/list", requireAuth, async (req, res) => {
  ...
  if (search) {
    values.push(`%${search}%`);
    conditions.push(
      `(p.name ILIKE $${values.length} OR p.sku ILIKE $${values.length} OR p.slug ILIKE $${values.length})`,
    );
  }
...
router.get("/", async (req, res) => {
  ...
  if (search) {
    values.push(`%${search}%`);
    conditions.push(
      `(p.name ILIKE $${values.length} OR p.sku ILIKE $${values.length} OR p.description ILIKE $${values.length})`,
    );
  }
  if (category) {
...
router.patch("/:id", requireAuth, async (req, res) => {
  ...
  if (!Object.keys(changes).length && images === undefined)
  ...
  try {
    ...
    if (entries.length) {
      const values = entries.map(([key, value]) =>
        key === "applications" ? JSON.stringify(value) : value,
      );
    ...
    if (images !== undefined)
      await replaceImages(
        ...
        product.id,
        images,
        changes.imageAlt ?? product.image_alt,
      );
    await client.query("COMMIT");
    res.json({
      product: { ...product, ...(images !== undefined ? { images } : {}) },
    });
  } catch (error) {
    await client.query("ROLLBACK");
    if (!sendWriteError(error, res)) throw error;
