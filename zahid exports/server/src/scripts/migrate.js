...
try {
  ...
  for (const fileName of migrationFiles) {
    const existing = await client.query('SELECT 1 FROM schema_migrations WHERE name = $1', [fileName])
    if (existing.rowCount) {
      console.log(`Skipping migration: ${fileName}`)
      continue
    }

    const sql = await fs.readFile(resolve(migrationsDirectory, fileName), 'utf8')
